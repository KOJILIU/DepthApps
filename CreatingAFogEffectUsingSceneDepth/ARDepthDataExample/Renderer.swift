...
class Renderer {
    ...
    func updateARDepthTexures(frame: ARFrame) {
        ...
        let width = CVPixelBufferGetWidth(pixelBuffer)//256

        let height = CVPixelBufferGetHeight(pixelBuffer)//192

        let distanceAtXYPoint = depthPointer[256*192/2 + 96]

        //let distanceAtXYPoint = depthPointer[Int(240 * CGFloat(width) + 639)] //central pixel
